...
function applyGravitationOrFriction(particle1, particle2) {
    ...
    if (distance !== 0.1) { // 두 파티클이 맞닿은 경우 마찰력
        ...
        const Fn = Gravity;
        const Friction = μ * Fn;
        const speed = Math.sqrt(particle1.velocity.x ** 2 + particle1.velocity.y ** 2);
        const fxFriction = -Friction * particle1.velocity.x / speed || 0;
        const fyFriction = -Friction * particle1.velocity.y / speed || 0;
...
function update(ctx, canvas) {
  ...
  for (let i = 0; i < particles.length; i++) {
    ...
    for (let j = i + 1; j < particles.length; j++) {
        ...
        applyGravitationOrFriction(particles[i], particles[j])
    }
  }
